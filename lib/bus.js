...
module.exports = function bus(conn, opts) {
  ...
  this.export = function(name, path, iface, nameFlags) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let doExport = function() {
        if (that._nameRequests[name]) {
          let request = that._nameRequests[name];
          request.then(() => {
            if (that._names[name]) {
        ...
        that._nameRequests[name] = new Promise((resolve, reject) => {
          if (!that._names[name]) {
            that.requestName(name, nameFlags, (err, result) => {
              if (err) {
                delete that._nameRequests[name];
                reject(err);
              } else if (result === constants.DBUS_REQUEST_NAME_REPLY_EXISTS) {
                delete that._nameRequests[name];
                reject(new NameExistsError(`The name already has an owner: ${name}`));
              } else {
        ...
        resolve(that._nameRequests[name]);
      }

      if (that._releaseRequests[name]) {
        that._releaseRequests[name].then(doExport);
      } else {
        doExport();
      }
    });
  ...
  this.unexportName = function(name) {
    ...
    if (that._releaseRequests[name]) {
      return that._releaseRequests[name];
    }

    let doUnexportName = function() {
      let nameRequest = that._nameRequests[name];
      delete that._nameRequests[name];

      let nameObj = that._names[name];

      that._releaseRequests[name] = new Promise((resolve, reject) => {
        let nameRequest = that._nameRequests[name];
    ...
    return new Promise((resolve, reject) => {
      if (that._nameRequests[name]) {
        that._nameRequests[name].then(function() {
          doUnexportName().then(() => resolve());
        });
      } else {
        doUnexportName().then(() => resolve());
      }
