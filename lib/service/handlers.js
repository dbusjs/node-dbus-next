...
function sendServiceError(bus, msg, errorMessage) {
  bus._send(Message.newError(msg, 'com.github.dbus_next.ServiceError', `Service error: ${errorMessage}`));
  return true;
}

function handleIntrospect(bus, msg, name, path) {
  bus._send(Message.newMethodReturn(msg, 's', [name._introspect(path)]));
}
...
function handleGetProperty(bus, msg, name, path) {
  ...
  // possibilities).
  if (!iface) {
    bus._send(Message.newError(msg, INVALID_ARGS, `No such interface: '${ifaceName}'`));
    return;
  }
  ...
  if (options === null) {
    bus._send(Message.newError(msg, INVALID_ARGS, `No such property: '${prop}'`));
    return;
  }
  ...
  try {
    ...
    if (e.name === 'DBusError') {
      bus._send(Message.newError(msg, e.type, e.text));
    } else {
  ...
  if (propertyValue instanceof DBusError) {
    bus._send(Message.newError(msg, propertyValue.type, propertyValue.text));
    return true;
  ...
  if (!(options.access === ACCESS_READWRITE ||
    ...
    bus._send(Message.newError(msg, INVALID_ARGS, `Property does not have read access: '${prop}'`));
  }

  let body = new Variant(options.signature, propertyValue);

  bus._send(Message.newMethodReturn(msg, 'v', [body]));
}
...
function handleGetAllProperties(bus, msg, name, path) {
  ...
  if (iface) {
    ...
    for (let k of Object.keys(properties)) {
      ...
      if (value instanceof DBusError) {
        bus._send(Message.newError(msg, value.type, value.text));
        return true;
  ...
  bus._send(Message.newMethodReturn(msg, 'a{sv}', [result]));
}
...
function handleSetProperty(bus, msg, name, path) {
  ...
  if (!iface) {
    bus._send(Message.newError(msg, INVALID_ARGS, `Interface not found: '${ifaceName}'`));
    return;
  }
  ...
  if (!(options.access === ACCESS_WRITE || options.access === ACCESS_READWRITE)) {
    bus._send(Message.newError(msg, INVALID_ARGS, `Property does not have write access: '${prop}'`));
  }

  if (value.signature !== options.signature) {
    bus._send(Message.newError(msg, INVALID_ARGS, `Cannot set property '${prop}' with signature '${valueSignature}' (expected '${options.signature}')`));
    return;
  }
  ...
  bus._send(Message.newMethodReturn(msg, '', []));
}
...
function handleStdIfaces(bus, msg, name) {
  ...
  if (!isInterfaceNameValid(ifaceName)) {
    bus._send(Message.newError(msg, INVALID_ARGS, `Invalid interface name: '${ifaceName}'`));
    return true;
  }

  if (!isMemberNameValid(member)) {
    bus._send(Message.newError(msg, INVALID_ARGS, `Invalid member name: '${member}'`));
    return true;
  }

  if (!isObjectPathValid(path)) {
    bus._send(Message.newError(msg, INVALID_ARGS, `Invalid path name: '${path}'`));
    return true;
  }
  ...
  if (ifaceName === 'org.freedesktop.DBus.Introspectable' &&
    ...
    if (member === 'Ping' && !signature) {
      bus._connection.message({
        type: constants.messageType.METHOD_RETURN,
        serial: bus._serial++,
...
function handleMessage(msg, bus) {
  ...
  for (let m of Object.keys(methods)) {
    ...
    if (method.name === member && method.inSignature === signature) {
      ...
      bus._send(Message.newMethodReturn(msg, method.outSignature, result));
      return true;
    }
  }
