...
function handleGetProperty(bus, msg, name, path) {
  ...
  try {
    ...
    if (e.name === 'DBusError') {
      ...
      bus.sendError(msg, 'com.github.dbus_next.ServiceError', `The service threw an error.\n${e.stack}`);
    }
    return true;
  }
  ...
  for (let m of Object.keys(methods)) {
    ...
    if (method.name === member && method.inSignature === signature) {
      ...
      if (result === undefined) {
        ...
        throw new Error(`method ${iface.$name}.${method.name} was not expected to return a result.`);
      } else if (!Array.isArray(result)) {
        throw new Error(`method ${iface.$name}.${method.name} expected to return multiple arguments in an array (signature: '${method.outSignature}')`);
      }
