...
class ServiceObject {
  ...
  _propertiesChangedHandler(changedProperties, invalidatedProperties) {
    ...
    that.bus._send(Message.newSignal(that.path, 'org.freedesktop.DBus.Properties', 'PropertiesChanged', 'sa{sv}as', body));
  }
  ...
  addInterface(iface) {
    ...
    let signalHandler = function(options, result) {
      ...
      that.bus._send(Message.newSignal(that.path, iface.$name, name, signature, result));
    }
