...
DBusBuffer.prototype.readSimpleType = function readSimpleType(t) {
  ...
  switch (t) {
    ...
    case 'x':
      //signed
      this.align(3);
      word0 = JSBI.BigInt(this.readInt32());
      word1 = JSBI.BigInt(this.readInt32());

      let word1Stripped = JSBI.bitwiseAnd(word1, JSBI.bitwiseNot(JSBI.leftShift(JSBI.BigInt(1), JSBI.BigInt(31))));

      let data = JSBI.add(JSBI.leftShift(word1Stripped, JSBI.BigInt(32)), word0);

      if (!JSBI.equal(word1Stripped, word1)) {
        data = JSBI.multiply(data, JSBI.BigInt(-1));
      }
    ...
    case 't':
      ...
      word0 = JSBI.BigInt(this.readInt32());
      word1 = JSBI.BigInt(this.readInt32());
      let result = JSBI.add(JSBI.leftShift(word1, JSBI.BigInt(32)), word0);
