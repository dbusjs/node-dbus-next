...
class ProxyObject {
   ...
   * remove only the listeners that includes a certain path and of specific event type
   * example : 
   * event -> "{"path":"/org/bluez/hci0/dev_XX_XX_XX_XX_XX_XX","interface":"org.freedesktop.DBus.Properties","member":"PropertiesChanged"}"
   * removeAllEventListeners('PropertiesChanged' , "/org/bluez/hci0/dev_XX_XX_XX_XX_XX_XX"); 
   */
  removeAllEventListeners(eventType, eventPath) {
    ...
    eventNames.forEach((event) => {
      ...
      if (event.includes(eventType) && event.includes(eventPath)) {
