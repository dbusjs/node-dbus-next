...
class ProxyObject {
  ...
  _callMethod(iface, member, inSignature, outSignature, ...args) {
    ...
    /**
   * remove listeners from _signals
   * avoiding possible memory leaks from too many event listeners
   * @param {string} eventName
   * if an event name (for example : 'PropertiesChaged')
   * is passed, remove listeners of only this type of events
   * if nothing is passed , remove all listeners
   */
  removeEventListeners(eventName) {
    const events = this.bus._signals.eventNames();
    events.forEach(event=>{
      try {
        if (eventName === null || eventName === undefined) {
          // remove ALL the listeners for ALL the events
          this.bus._signals.removeAllListeners(event);
        } else if (event.includes(eventName)) {
          // remove only events of a certain type
          // example : removeEventListeners('PropertiesChanged')
          this.bus._signals.removeAllListeners(event);
        }
      } catch (err) {
        throw new Error(`Error removing listener ${event} : ${err.message}`);
      }
    });
  }
